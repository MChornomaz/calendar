...
export class EventInfoCardComponent {
  ...
  edit() {
    if (this.calendarEvent()) {
      ...
      this.appointmentModalService.openAppointmentDialog(modalData).subscribe(() => this.close());
    }
  }

  delete() {
    if (this.calendarEvent()) {
      this.calendarEventService.deleteEvent((this.calendarEvent() as CalendarEvent).id).subscribe(() => this.close(this.calendarEvent() as CalendarEvent));
    }
  }
